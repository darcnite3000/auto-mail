import { staticContent, flatTag, mapOpt } from './utils'
...
function mapOrWrap(children, wrapper) {
  return options =>
...
function spreadChildren(children, options = {}) {
...
function flattenArray(array, el) {
...
export function tag(id = 'div', props, ...children) {
  ...
  children = children.reduce(flattenArray, [])
...
export function text(props, childText) {
  if (arguments.length === 1) {
    return staticContent(props)
  }
  if (props.replacements) {
    const childProps = { ...props }
    delete props['replacements']
    return divWrap(props)(replacedText(childProps, childText))
  }
  return divWrap(props)(staticContent(childText))
}
...
export function paddedCellText(
  ...
  if (inner)
    return td(
      ...
      text(inner, childText)
    )
  return td(
    {
      style
    },
    staticContent(childText)
  )
}
